...
Статус: **CURRENT / current lifecycle contract carried into v7.7 r1** (обновлено 2026-09-25). Kernel-v1 remains the common lifecycle foundation; all seven accepted instances now use kernel-v2 physical Capital Goods backing.
Модель: `Orbital Economy v7.7 r1 — Construction Materials`, SHA-256 `5bbc29b6e18caa64ec22267892b6cd0669649722c8fc029d8dba43a77a34d5a1`. Capital Lifecycle structure is inherited from accepted v7.5.1.
Машинная форма контракта: plugin `capital_lifecycle_kernel` в `validation/validation-v7.7.json`; проверяющий код `lab/src/lifecycle_conformance.js` (static) и `lab/src/checks.js` (runtime). При расхождении машинная форма и accepted code имеют приоритет.
