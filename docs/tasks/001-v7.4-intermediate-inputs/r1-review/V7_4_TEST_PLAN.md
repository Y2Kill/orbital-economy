# v7.4 — Intermediate Inputs — test plan (r1.1)
...
Шок: `A Electronics` спрос × 4 постоянно с дня 360 (как Mode 14, но со связью). Ожидаемая цепочка: спрос на электронику ↑ → спрос Electronics на металл ↑ → дефицит металла у A → плавка ↑ → строительство refinery → энергия ↑ → строительство генерации.

| # | Ожидание | Проверка | Порог |
|---|---|---|---|
| 19.1 | промежуточный спрос растёт | metric last `A Electronics Metal Input Demand` @1080 ≥ 1.5 × @359.75 | [fix] |
| 19.2 | межотраслевая цепочка капитала | event_order: `A Electronics Metal Input Demand` > 1.2 × pre-shock → `A Refinery Expansion` > 0 [360,1080] → `A Power Generation Expansion` > 0 [360,1080] | [fix] порядок |
| 19.3 | fulfillment проседает, но система адаптируется | metric min `A Electronics Metal Input Fulfillment` [360,1080] < 1 **и** last @1080 ≥ min + 0.05 (тренд к восстановлению) | [fix] |
| 19.4 | нет взрыва мощностей | `A Refinery Installed Capacity` @1080 ≤ 3 × @360 | [calib] множитель |
| 19.5 | сравнение с Mode 14 | таблица в отчёте: выпуск электроники A, unserved energy A, installed refinery A на дни 720/1080 в Mode 14 и Mode 19; объяснить различия связью | отчёт |
