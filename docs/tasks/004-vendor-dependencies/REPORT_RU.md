...
1. **Литерал версии в web-манифесте.**  
   `lab/src/workspace.js` теперь импортирует `EXPECTED_ENGINE_VERSION` из `engine.js`; поле `engine.version` в `web_batch_manifest.json` формируется из этой единственной ожидаемой константы вместо отдельного литерала `'9.0.0'`.

2. **Формат отказа engine pin.**  
   `assertEngineVersion()` сохранена бросающей функцией, чтобы negative QA мог проверять её напрямую. Стартовая проверка в `engine.js` теперь перехватывает исключение, печатает `[ERROR] <исходный текст сообщения>` без stack trace и завершает процесс с кодом `2`. Текст диагностики не сокращён и по-прежнему содержит ожидаемую и фактическую версии / путь metadata.

3. **Ожидаемая версия в policy QA.**  
   `lab/src/policy_qa.js`: эталонная `basePolicy.engine.version` теперь использует `EXPECTED_ENGINE_VERSION`. Фактический `ENGINE_VERSION` остаётся только в synthetic comparison report, где он описывает реальный runtime.

## Что не запускалось в раунде 2

Как и в раунде 1, из этой API-среды не запускались guard и стенд: `check_branch`, self-tests, `RUN_LAB`, `CHECK_CANDIDATE`. Правки проверены чтением изменённых путей и согласованием expected/factual engine semantics. SUMS не пересобирались согласно `sums_by: reviewer`.
