...
Проверка самих инструментов: `bash tools/selftest.sh` (песочница во временной папке; 25 сценариев — честная поставка, типовые нарушения и режим --in-progress; реальный репозиторий и origin не трогаются). Прогонять после любой правки в `tools/`.
