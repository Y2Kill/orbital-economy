...
- **Уточнение 2026-09-26 (после приёмки).** Утверждение ниже верно только для Modes 27–29 и спокойных сценариев (17, 21). Полный прогон всех 30 Modes показал: B строит под стимулом — в Modes 18–20 и 23–26 растут её Refinery и генерация (в Mode 24 — через повторно использованный всплеск транспортного спроса); потребности нет только в спокойных сценариях, потому что B стартует с избыточными мощностями и сворачивает их. Следствие для v7.7: путь стройматериалов B проверен только на нулях; сценарий со стимулом при включённых стройматериалах — в v7.7.1.
- **B не строит** *(исходный текст, см. уточнение выше)*. В accepted-динамике колония B ни в одном сценарии не расширяет мощности (её Refinery выбывает 28 → 8.5 к дню 1080), поэтому спрос B на стройматериалы равен 0 во всех Modes 27–29 (skeleton). Сектор B существует и зеркален, но простаивает. Проверки вида «B производит стройматериалы» заведомо ложны (урок задачи 003, контракт §8 п. 6). Для B проверяется покой: fulfillment = 1, запасы нетронуты.
